...
- AI 页显示 Codex 周额度剩余比例、准确刷新时间、六个真实任务标题与逐项状态；Claude Code 只显示本机会话
- Codex Micro 原生 HID 控制：六任务、快速模式、批准、拒绝、续开、语音、发送、四向和旋钮
- 媒体页显示真实标题、艺人、播放器、播放进度、音量与静音状态，并提供六个系统媒体键
- Wi-Fi Station、NVS 配网、断线重试和阿里云 NTP 对时
- 证书校验 HTTPS、PSRAM 响应缓冲和全局串行握手
- 和风天气当前状态、未来 6 小时、今明两天与有效预警
- 上证、深证官方接口提供六个常见 A 股指数
- 设置页显示 Wi-Fi 名称/信号/IP、蓝牙绑定/MTU/心跳、公共数据新鲜度和媒体隐私状态
- 设备诊断页每 5 秒更新设备、内存、连接、存储卡和日志状态，支持即时刷新与隐私安全的诊断快照
...
当前固件镜像约 2.12MB；采用两个 6MB OTA 应用分区，剩余约 66%，板载 SPIFFS 约 3.9MB。microSD 用于长期日志、截屏和大型资源，应用程序仍运行在板载 Flash 中。

默认配置为已经完成实机验证的 `ESP32-S3-Touch-LCD-4.3B`。

正常构建默认关闭 `DESK_BRINGUP_DISPLAY_ON` 和 `DESK_ALLOW_APP_ENROLLMENT`：开机保持熄屏，只有通过应用认证后才亮屏；断开、心跳超时和 Mac 锁屏仍会熄屏并禁用触摸。全新设备或 NVS 已擦除时，只在受控登记构建中临时开启 `DESK_ALLOW_APP_ENROLLMENT`，登记完成后立即恢复关闭。联调常亮开关不再绕过运行期隐私动作。
